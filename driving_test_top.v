...
// Generate single-clock pulses for audio events
reg [3:0] coin_count_prev;
reg       collision_prev;
wire      coin_pulse  = (coin_count > coin_count_prev);
wire      crash_pulse = collision & ~collision_prev;

always @(posedge CLOCK_50) begin
    if (!rst_n) begin
        coin_count_prev <= 4'd0;
        collision_prev  <= 1'b0;
    end else begin
        coin_count_prev <= coin_count;
